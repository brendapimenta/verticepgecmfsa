...
import {
  ...
  FileText, AlertCircle, CheckCheck, ArrowRightLeft, Calendar, Sun, Moon, Gavel, Building2, Download
} from 'lucide-react';
...
export const AppLayout: React.FC = () => {
  ...
  // Sala Principal alert modal state
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      sonnerToast.custom((toastId) => (
        <div onClick={() => { marcarLidaRef.current(notif.id); navigateRef.current(route); sonnerToast.dismiss(toastId); }}
  ...
  const handleFecharAlertaPopup = () => {
    if (alertaPopup) marcarNotificacaoLida(alertaPopup.id);
    setAlertaPopup(null);
  };
  ...
  const navItems = [
    ...
    { to: '/chat', label: 'Chat', icon: MessageSquare, roles: ['administrador', 'sala_principal', 'presidente', 'sala_espera'] },
    { to: '/notificacoes', label: 'Notificações', icon: Bell, roles: ['administrador', 'sala_principal', 'presidente', 'sala_espera'] },
    { to: '/pauta-despacho', label: 'Pauta para Despacho', icon: Gavel, roles: ['administrador', 'sala_principal', 'presidente'] },
          ...
          return (
            <Link key={item.to} to={item.to} onClick={() => setSidebarOpen(false)}
              className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200",
                active ? "bg-sidebar-accent text-sidebar-foreground border-l-[3px] border-l-sidebar-primary shadow-sm" : "text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent")}>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      <aside className="hidden md:flex w-64 flex-col sidebar-gradient border-r border-sidebar-border"><SidebarContent /></aside>
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div className="absolute inset-0 bg-black/50" onClick={() => setSidebarOpen(false)} />
          <aside className="relative w-64 h-full sidebar-gradient animate-slide-in"><SidebarContent /></aside>
        </div>
      )}
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-border gap-3 bg-card">
          <button onClick={() => setSidebarOpen(true)} className="md:hidden p-2 rounded-lg hover:bg-muted"><Menu className="w-5 h-5" /></button>
          {isAdmin && (
          ...
          <div className="flex items-center gap-3">
            <button onClick={toggleTheme} className="p-2 rounded-lg hover:bg-muted transition-colors" title={theme === 'light' ? 'Ativar modo escuro' : 'Ativar modo claro'}>
              {theme === 'light' ? <Moon className="w-5 h-5 text-muted-foreground" /> : <Sun className="w-5 h-5 text-muted-foreground" />}
            </button>
            {isSalaPrincipal && (
              <button onClick={() => setAlertaOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors">
        ...
        {isViewingAs && (
          <div className="flex items-center justify-between px-4 md:px-6 py-2 border-b border-border text-sm bg-primary/10">
            <span className="flex items-center gap-2 text-muted-foreground"><Eye className="w-4 h-4" />Você está visualizando como: <strong className="text-foreground">{perfilLabels[perfilVisual]}</strong></span>
            <button onClick={() => setPerfilVisual('administrador')} className="text-xs font-medium underline hover:no-underline text-muted-foreground hover:text-foreground">Voltar para Administrador</button>
          </div>
        )}
        <div className="flex-1 overflow-auto p-4 md:p-6"><Outlet /></div>
        <footer className="px-4 md:px-6 py-2 border-t border-border text-center">
          <p className="text-[10px] text-muted-foreground/60 uppercase tracking-wider">© 2026 VÉRTICE TECNOLOGIA INSTITUCIONAL &nbsp;·&nbsp; CONTROLE, ESTRATÉGIA E PRECISÃO NA GESTÃO PÚBLICA.</p>
        </footer>
      </main>

      {/* Modal: Sala Principal envia alerta */}
      <Dialog open={alertaOpen} onOpenChange={setAlertaOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-red-500" />Enviar alerta ao Presidente</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <label className="text-xs font-medium text-muted-foreground">Mensagem de alerta</label>
            <Textarea value={alertaMensagem} onChange={e => setAlertaMensagem(e.target.value)} placeholder="Descreva rapidamente o que precisa de atenção imediata..." rows={3} />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setAlertaOpen(false); setAlertaMensagem(''); }}>Cancelar</Button>
            <Button onClick={handleEnviarAlerta} disabled={!alertaMensagem.trim()} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Enviar alerta</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!alertaPopup} onOpenChange={open => !open && handleFecharAlertaPopup()}>
        <DialogContent className="sm:max-w-md border border-destructive bg-card">
          <DialogHeader><DialogTitle className="text-lg font-bold flex items-center gap-2 text-destructive"><AlertTriangle className="w-5 h-5" />⚠ ALERTA DA SALA PRINCIPAL</DialogTitle></DialogHeader>
          <div className="py-3">
            <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">{alertaPopup?.mensagem_resumo.replace('Alerta da Sala Principal: ', '')}</p>
            {alertaPopup && (<p className="text-[11px] text-muted-foreground mt-3">{new Date(alertaPopup.criado_em).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>)}
          </div>
          <DialogFooter><Button onClick={handleFecharAlertaPopup}>Entendido</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
